...
public class Sliding_adapter extends PagerAdapter {
    ...
    // Добавление элементов в слайд панель
    ...
    //Получение их количества
    @Override
    public int getCount() {
        return data_list.size();
    }

    //Установка фотографий в блоки
    @Override
    ...
    //Ниже доп методы,не значительные. Это смена состояния, он не используется, можно удалить
    //И сравнение данного представления с объектом
    @Override
    ...
    //Обычный гетер и сетер, для получения инфы в панеле. (Желательно вынести в отдельный класс)
    //Так как он не "особо большой", для экномии вермени оставил тут
